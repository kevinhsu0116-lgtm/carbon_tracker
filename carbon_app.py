...
st.markdown("""
    ...
    .strategy-card { background-color: white; padding: 15px; border-radius: 10px; border-left: 5px solid #2E7D32; margin-bottom: 10px; }
    </style>
...
STRATEGY_DATA = {
    "food": {
        ...
        "points": [
            ...
            "2. 低碳加工技術替代：選擇加工層次較低的產品。若改採當季常溫在地蔬菜，就能省下背後巨大的電力間接排放。",
            "3. 物流整合與去節點化：要求供應商採用大包裝進場、或原箱配送，減少物流中心停留與二次包裝產生的碳足跡。",
            "4. 高生產效率選擇: 挑選採用智慧農場、再生水灌的供應商。這些商品在原料取得階段的碳強度較低。"
        ],
        "perfect": "5. 100分"
    },
    "disposable": {
        ...
        "points": [
            ...
            "2. 丟對垃圾桶：確保塑料與紙張進入循環路徑。當廢棄物變成再生原料，就從碳排放變成資源供應。",
            "3. 可堆肥材質：針對無法回收的耗材，轉向「工業可堆肥」材質，讓垃圾從掩埋焚化轉變為再利用。",
            "4. 減少包裝: 選擇裸裝或簡約包裝商品，降低材料生產與廢棄處理帶來的聯動排放。"
        ],
        "perfect": "5. 不錯嘛"
    },
    "energy": {
        "title": "三. 針對能源使用:",
        "desc": "就算你很省電，但台灣電力係數高。作法: 轉向高能效設備與行為，提高每度電的產生效率。",
        "points": [
            "1. 高能效比：換掉老舊電器，採用具備一級能效設備。能源轉化率高，可減少 30%-50% 的電力間接排放。",
            "2. 能源轉型: 監督並支持政府加速低碳天然氣與再生能源配比。當國家電網電力係數下降，你的排碳就變低。",
            "3. 行為優化：消除待機損耗。透過自動排程系統，在非使用時段自動切斷電源。",
            "4. 支持環保: 優先選擇綠電店家，例如支持已經採購綠電，或認真環保的店。"
        ],
        "perfect": "5. 讚"
    },
    "transport": {
        ...
        "points": [
            "1. 搭大眾運輸: 能一次載運多人，將過程中的碳排平分下去，減少各移動帶來的排放。",
            "2. 輕量化：車上每增加45公斤負載，油耗增加約 1%-2%。移除長期堆積的物品。",
            "3. 定期保養載具: 確保載具處於低阻力、高燃燒狀態，提高能源使用並維持引擎最高效。",
            "4. 智慧路徑規劃: 根據路徑距離選擇最高效工具。使用導航避開塞車，穩定時速就是最高效的移動。"
        ],
        "perfect": "5. 好棒"
    }
}
...
with tab2:
    if 'res' in st.session_state:
        ...
        st.header(f"今日總評分：{'⭐' * (1 if r['total']>42 else 3 if r['total']>13 else 5)}")
        ...
        st.divider()
        st.header("📋 盤查應對策略 (效率導向)")
        st.info("減碳不代表要過苦日子，而是用更聰明、高效的方式過日子。")

        # 動態策略顯示邏輯
        # 定義四項目的評分 (這可以根據你的係數微調門檻)
        item_scores = {
            "food": _get_score(r['food'], 5, 15),
            "disposable": _get_score(r['disp'], 0.5, 2),
            "energy": _get_score(r['home'], 3, 10),
            "transport": _get_score(r['move'], 1, 5)
        }
        ...
        for key, score in item_scores.items():
            ...
            if score == 5:
                ...
                show_count = 5 - score # 1分顯4點, 2分顯3點...
...
with tab3:
    try:
        ...
        if res.data:
            ...
            st.metric("平均排放", f"{round(df['total'].mean(), 2)} kg")
